...
<div className={`cart ${state.cart.length > 0 ? 'show' : ''}`}>
  ...
  {state.cart.length > 0 ? (
    ...
    <p>Корзина пуста</p>
      ...
      <Description children='используется для управления сложным состоянием, особенно когда оно содержит несколько подзначений или когда следующее состояние зависит от предыдущего. Вместо множества вызовов useState, useReducer позволяет централизовать логику обновления состояния в одном месте — с помощью редюсера (функции, принимающей текущее состояние и действие, и возвращающей новое состояние). Это упрощает управление состоянием в крупных компонентах и делает логику более предсказуемой и тестируемой.'/>
    </div>
  );
}
